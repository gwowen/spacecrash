...
void Render() {
  ...
  //render the background, only tiles within camera view
  int first_tile = (int) floorf( g_camera_offset / G_HEIGHT );
  
  for( int i = first_tile; i < first_tile + 2; i++ ) {
    CORE_RenderCenteredSprite(
			      vsub( vadd( vmake( G_WIDTH / 2.0f, G_HEIGHT / 2.0f ),
					  vmake( 0.f, (float)i * G_HEIGHT )),
				    vmake( 0.f, g_camera_offset ) ),
...
void RunGame() {
  ...
  for( int i = MAX_ENTITIES - 1; i>=0; i-- ) {
    if( g_entities[i].type != E_NULL ) {
      ...
      //remove entities that have fallen off the screen
